...
void cRocket::update(float deltaTime)
{
	...
	if (m_InputMgr->isKeyDown(VK_RIGHT)) //moves player right
	{
		spritePos2D.x += 7.0f;
	}
	if (m_InputMgr->isKeyDown(VK_LEFT)) //moves player left
	{
		spritePos2D.x -= 7.0f;
	}
	...
	if (m_InputMgr->isKeyDown(VK_SPACE))
	{
		if (x > 0)
		{
			...
			theRocketBullets[numBullets]->setSpriteTranslation(glm::vec2(5.0f, 25.0f));
			...
			theRocketBullets[numBullets]->setBulletVelocity(glm::vec2(0.0f, 1000.0f));
	...
	glm::vec2 spriteVelocityAdd = glm::vec2(0.0f, 2.0f);
...
// checks for collisions in a method so that score can be added

int cRocket::newScore(int score)
{
	for (vector<cBullet*>::iterator bulletIterartor = theRocketBullets.begin(); bulletIterartor != theRocketBullets.end(); ++bulletIterartor)
	{
